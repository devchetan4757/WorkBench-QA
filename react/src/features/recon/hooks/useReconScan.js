...
export function useReconScan() {
  ...
  // manual "fetch from any URL" wordlist source — separate from the preset
  // dropdown above so its own loading/error state doesn't fight with it
  const [customWordlistUrl, setCustomWordlistUrl] = useState("");
  const [customFetchStatus, setCustomFetchStatus] = useState("idle"); // idle | loading | done | error
  const [customFetchError, setCustomFetchError] = useState("");
  const [customFetchMeta, setCustomFetchMeta] = useState(null);
  ...
  // ── Fetch a wordlist from any URL you type in, not just the presets ──
  const fetchCustomWordlist = useCallback(async () => {
    const target = customWordlistUrl.trim();
    if (!target) return;
    ...
    try {
      ...
      const lines = text
        ...
        .filter((l) => l && !l.startsWith("#"));
      setWordlist(lines.join("\n"));
      setSelectedWordlistKey("Custom (type below)");
      setCustomFetchMeta({ count: lines.length });
      setCustomFetchStatus("done");
    } catch (e) {
      setCustomFetchError(`Failed to fetch: ${e.message}`);
